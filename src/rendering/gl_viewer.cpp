...
void main() {
    ...
    GLViewer::GLViewer(const std::string& title) : window_(nullptr), window_title_(title) {
    ...
    bool GLViewer::init() {
        ...
        // Set up initial view
        projection_matrix_ =
            glm::perspective(glm::radians(45.0f), (float)width_ / height_, 0.1f, 100.0f);
        update_view_matrix();
    ...
    void GLViewer::update_view_matrix() {
        ...
        glm::vec3 eye(camera_distance_ * sin(camera_angle_y_) * cos(camera_angle_x_),
    ...
    void GLViewer::update_points_with_colors(const std::vector<cv::Point3f>& points,
        ...
        for (size_t i = 0; i < num_points; ++i) {
            ...
            colors_.push_back(glm::vec3(colors[i][2] / 255.0f,  // BGR to RGB
                                        colors[i][1] / 255.0f, colors[i][0] / 255.0f));
        }
    }
    ...
    void GLViewer::draw_axes() {
        ...
        glBufferData(GL_ARRAY_BUFFER, axis_points.size() * sizeof(glm::vec3), axis_points.data(),
                     GL_DYNAMIC_DRAW);
        ...
        glBufferData(GL_ARRAY_BUFFER, axis_colors.size() * sizeof(glm::vec3), axis_colors.data(),
                     GL_DYNAMIC_DRAW);
    ...
    void GLViewer::init_grid() {
        ...
        glBufferData(GL_ARRAY_BUFFER, grid_points.size() * sizeof(glm::vec3), grid_points.data(),
                     GL_STATIC_DRAW);

        glBindBuffer(GL_ARRAY_BUFFER, vbo_grid_colors_);
        glBufferData(GL_ARRAY_BUFFER, grid_colors.size() * sizeof(glm::vec3), grid_colors.data(),
                     GL_STATIC_DRAW);
    }
    ...
    bool GLViewer::render() {
        ...
        if (!points_.empty()) {
            ...
            // Update vertex positions
            glBindBuffer(GL_ARRAY_BUFFER, vbo_points_);
            glBufferData(GL_ARRAY_BUFFER, points_.size() * sizeof(glm::vec3), points_.data(),
                         GL_DYNAMIC_DRAW);
            glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, nullptr);
            glEnableVertexAttribArray(0);
            ...
            // Update vertex colors
            glBindBuffer(GL_ARRAY_BUFFER, vbo_colors_);
            glBufferData(GL_ARRAY_BUFFER, colors_.size() * sizeof(glm::vec3), colors_.data(),
                         GL_DYNAMIC_DRAW);
            glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, nullptr);
            glEnableVertexAttribArray(1);
