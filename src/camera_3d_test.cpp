...
int main() {
    ...
    while (!viewer.should_close()) {
        ...
        if (mapper->has_cloud()) {
            ...
            points_3d =
                features_on_plane(result.curr_points, default_intrinsics(frame.size()), 1.0f);
        }
        ...
        for (const auto& [id, trail] : feature_trails) {
            ...
            for (size_t i = 0; i < trail.size() - 1; ++i) {
                ...
                cv::line(display, trail[i], trail[i + 1], cv::Scalar(0, 255 * opacity, 0), 1,
                         cv::LINE_AA);
            }
        }
        ...
        cv::rectangle(dimmed, cv::Rect(0, 0, overlay_width, overlay_height), cv::Scalar(0, 0, 0),
                      -1);
        cv::addWeighted(dimmed, alpha, overlay, 1.0 - alpha, 0, overlay);

        // Add text overlays
        cv::putText(display, "FPS: " + std::to_string((int)fps), cv::Point(10, 30),
                    cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 255), 1, cv::LINE_AA);

        cv::putText(display, "Tracked: " + std::to_string(result.num_tracked), cv::Point(10, 60),
                    cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 255), 1, cv::LINE_AA);

        cv::putText(display,
                    "Quality: " + std::to_string((int)(result.tracking_quality * 100)) + "%",
